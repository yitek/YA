(function (factory) {
    ...
    Unittest_1.Unittest.Test("YA.Core", {
        "ValueObservable": (assert, info) => {
            let ob = new YA_core_1.ValueObservable();
        ...
        "ValueProxy": (assert, info) => {
            let target = { x: 1 };
            let proxy = new YA_core_1.ValueProxy((value) => (value === undefined) ? target.x : target.x = value);
        ...
        "ObjectProxy": (assert, info) => {
            ...
            let proxy = new YA_core_1.ObjectProxy((val) => val === undefined ? target : target = val, {
                fieldnames: ["name"],
            ...
            let propAge;
            try {
                YA_core_1.ValueProxy.gettingProxy = true;
                propAge = proxy.age;
            }
            finally {
                YA_core_1.ValueProxy.gettingProxy = false;
            }
            assert(true, propAge instanceof YA_core_1.ValueProxy, "当ValueProxy.gettingProxy开关打开时，属性返回的的是代理对象本身");
            let evtArgs;
        ...
        "ArrayProxy": (assert, info) => {
            let target = [14, 24, 38, 40];
            let proxy = new YA_core_1.ArrayProxy((val) => val === undefined ? target : target = val);
            ...
            assert(true, idx1 instanceof YA_core_1.ValueProxy, "可以获取到item代理");
            assert(true, idx2 instanceof YA_core_1.ValueProxy, "可以获取到item代理");
