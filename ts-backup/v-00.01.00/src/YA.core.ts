...
export function defineSingleonProperty(target:any,name:string|{[name:string]:(self)=>any},getter?:(self)=>any){
...
export function fulfillable(topic:string,subject?:any){
    if(!subject) subject = {};
...
export class Observable<TData> extends Subject<IChangeEventArgs<TData>> implements IObservable<TData>{
    ...
    constructor(init:IObservableIndexable<TData>|{(val?:TData):any}|TData,index?:any,initValue?:any){
        ...
        if(!this.$schema) this.$schema= this.$schema = new ObservableSchema(undefined);
    ...
    toString(){
        let  currentValue = this.get(ObservableModes.Value);
...
export class ObservableSchema<TData>{
    ...
    defineProp<TProp>(propname:string,initValue?:TProp,onSetting?:(value)=>any):ObservableSchema<TProp>{
        ...
        let private_prop_name = "$YA__" + propname + "__";
        let self = this;
...
export function loopar<T>(schema :any, index?:string, subject?:any):ObservableProxy<T>{
    if(schema instanceof Observable){
        ...
        for(let n in schema){
            schema = new ObservableSchema<T>(schema[n]);
            break;
        }
        if(!schema)schema = new ObservableSchema<T>(undefined);
    }
...
ElementUtility.createPlaceholder=(cmt?:string):any=>{
    let rs = document.createComment(cmt||"PLACEHOLDER");
...
function _createElement(
    ...
    if(t==="string"){
        ...
        if(ElementUtility.isElement(tag)){
            if(attrs)ElementUtility.appendChild(attrs as IElement,tag as any);
            return attrs;
...
export function createComponent(componentType:any,descriptor:INodeDescriptor,container?:IElement,ownComponent?:IComponent,opts?:ComponentCreationOpts):IElement[]|IElement|IComponent{
    ...
    try{
        ...
        if(typeof compInstance.render==='function'){
            ...
            if(compInstance.init) compInstance.init(descriptor);
...
function bindClientName(ownComponent,value,elem){
    ...
    elem["c-name"] = value;
    let existed = ownComponent["c-name"];
    if(!existed) {
...
function bindComponentAttr(compInstance:IComponent,propName:string,bindValue:any):boolean{
    ...
    if(propName==="_options"){debugger;}
    let propInfo =  meta.reactives?meta.reactives[propName]:null;
    let isOb = Observable.isObservable(bindValue);
    let actualBindValue = isOb?bindValue.get(ObservableModes.Value):bindValue;
...
function getContext():any{
    let ctx = this.$__YA_CONTEXT__;
    if(!ctx){
        let Context = function(){};
        Context.prototype = this.$parent?this.$parent.context:context;
        ctx = new Context();
...
export function reactive(type?:ReactiveTypes,schema?:ObservableSchema<any>|any,name?:string,obj?:any){
    ...
    if(!obj) return (schema as ObservableSchema<any>).createObservable(Default);
...
function bindDomCondition(elem:IElement,bindValue:any,vnode:INodeDescriptor,compInstance:IComponent,isShowPlaceholder:(val:any)=>boolean){
    let placeholder = ElementUtility.createPlaceholder((elem as any).id);
