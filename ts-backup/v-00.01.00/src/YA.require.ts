...
interface IRequireReady{
    ...
    isReady:boolean;
    isFailed:boolean;
    ready(callback:(value:any,sender?:IRequireReady)=>any):IRequireReady;
    fail(callback:(value:any,sender?:IRequireReady)=>any):IRequireReady;
}
...
(function(window){    
    ...
    function factory(initModules:{[name:string]:any},window:any,document?:HTMLDocument){
        ...
        class Ready implements IRequireReady{
            ...
            isReady:boolean;
            isFailed:boolean;
            constructor(statement:(ready:(value:any,sender?:Ready)=>void,fail:(err:any,sender:Ready)=>void)=>void){
        ...
        class Resource extends Ready implements IRequireResource{
            ...
            constructor(public url:string){
                super(function(ready,fail){
                    ...
                    elem.onerror =(err,ex)=>{console.error(err,ex,this);fail(err,self);}
        ...
        class RequireContext{
            ...
            constructor(public name:string,public deps:string[],public factory:Function){
                ...
                setTimeout(()=>{
                    ...
                    let args = [];
                    let wait_count=this.deps.length+1;//自己最后会调用一次tryResolve.
                    let tryResolve =()=>{
                        if((--wait_count)===0){
                            let success = false;
                            let result;
                    ...
                    for(const index in this.deps)((depname:string,idx:string)=>{
                        if(depname==="exports") {
                            ...
                            depModule.ready((value:any,mod:Module)=>{
                                args[idx] = value;
                                tryResolve();
                            });
                        }
