...
declare let Promise;
try{
    if(typeof Promise!=="function") (window as any).Promise = YA.Promise;
}catch(ex){
    (window as any).Promise = YA.Promise;
}
...
export class Component extends YA.Component{
    ...
    request(opts:string|IHttpRequestOpts,url?:any,data?:any):HttpRequest{
        ...
        if(this.$parent) return (this.$parent as IComponent).request(opts,url,data);
        if(url!==undefined){
            opts = {method:opts as any,url:url,data:data};
        }
        return new HttpRequest(opts as IHttpRequestOpts);
    }
...
export class Dropdown extends Component{
    ...
    init(descriptor:any){
        ...
        this._initItems(this.options,true).then(()=>this._initValueText());
    }

    private _initItems(options:any,returnPromise?:boolean){
        let t = typeof options;
        if(t==="object" && !options[HttpRequest.optsToken]){
            let items =  [];
            if(YA.is_array(options)){
                for(const item of options) items.push(item);
            }else{
            ...
            this._items = items;
            return returnPromise?new Promise((resolve)=>{
                resolve(this._items);
                if(this.$element){
                    ElementUtility.removeClass(this.$element as any,"waiting");
                    this._textEditor.disabled = true;
                }
            }):this._items;
        }else {
            
            this.request(options).then((optsData)=>{
                this._initItems(optsData);
                ElementUtility.removeClass(this.$element as any,"waiting");
                this._textEditor.disabled = true;
            });
        }
    ...
    private _initValueText(){
        ...
        this.subscribe("value",(e)=>{
            this._setDropdownValue(this.value);
        });
    }
    private _setDropdownValue(value){
        if(Observable.isObservable(value)) value = value.get(ObservableModes.Value);
        let rm = Observable.readMode;
        Observable.readMode = ObservableModes.Proxy;
        try{
            let opts = this._options.get(ObservableModes.Value);
            let index = 0;
            for(let n in opts){
                let opt = this._options[n];
                let val =opt[this.VALUE_FIELD||"Id"||"id"];
                if(val.get) val = val.get(ObservableModes.Value);
                if(value === val){
                    let lastIndex = (this.selectIndex as any).get(ObservableModes.Value);
                    (this.selectIndex as any).set(index);
                    let text = opt[this.TEXT_FIELD||"Text"||"text"];
                    (this.text as any).set(text);
                    if(this._textText){
                        this._textText.innerHTML = text;
                        let tbody = this._dropdown_list;
    ...
    private selectOpt(opt){
        let value = opt[this.VALUE_FIELD||"Id"||"id"];
        this.value(value);
    }
    render():any{
        debugger;
        let opt = YA.loopar(this._options);
        let fieldText = YA.loopar(this.fields);
        let fieldName = YA.variable("") as any;
        let thead = <thead if={this.fields}>
        <tr loop={[this.fields,fieldText,fieldName]}>
            <th>{fieldText}</th>
        </tr>
        </thead>;
        return <span className={["ya-dropdown",this.css]}>
            <input if={this.editable} type="text" c-name="$__textEditor__" class="ya-dorpdown-text" value={this.text} />
            <span if-not={this.editable} c-name="$__textText__">{this.text}</span>
            <table>
                
                <tbody loop={[this._options,opt]} c-name="$__dropdown_list__"><tr loop={[this.fields,fieldText,fieldName]} __opt_data={opt}>
                        <td>{opt[fieldName]}</td>
                </tr></tbody>
            </table>
        </span>;
...
export class Dropdown1 extends Component{
    ...
    private _setValue(value:any):Dropdown1{
